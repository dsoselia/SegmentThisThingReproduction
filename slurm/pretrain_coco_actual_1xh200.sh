#!/bin/bash
# MAE pre-training — actual full run, 1×H200.
# Grad accumulation auto-adjusts to maintain target batch size.
# 500K steps; ~288h estimated → uses --requeue to survive preemptions.
#
# Submit: sbatch slurm/pretrain_coco_actual_1xh200.sh

#SBATCH --job-name=stt-mae-act-1h
#SBATCH --output=logs/mae_actual_1xh200_%j.out
#SBATCH --error=logs/mae_actual_1xh200_%j.err

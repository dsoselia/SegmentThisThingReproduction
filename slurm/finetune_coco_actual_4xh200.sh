#!/bin/bash
# Segmentation fine-tuning — actual full run, 4×H200 (fastest config).
# 250K steps. Requires mae_coco_lr_actual/final.pth from pretrain.
#
# Submit: sbatch slurm/finetune_coco_actual_4xh200.sh
# With dependency: sbatch --dependency=afterok:<pretrain_jobid> slurm/finetune_coco_actual_4xh200.sh

#SBATCH --job-name=stt-seg-act-4h
#SBATCH --output=logs/seg_actual_4xh200_%j.out
#SBATCH --error=logs/seg_actual_4xh200_%j.err
